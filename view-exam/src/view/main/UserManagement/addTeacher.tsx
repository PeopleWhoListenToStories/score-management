      ...
      <div className={AddCss.item}>
        <Tabs defaultActiveKey="1" onChange={callback}>
          ...
          <TabPane tab="更新用户" key="2">
            <Form {...layout} name="nest-messages" onFinish={(values: any) => { AddUserStore.renewalUserAction(values.user_id, values.user_name, values.user_pwd, values.identity_id, avatar); formRefOnes.resetFields() }} initialValues={{ identity_text: '' }} form={formRefOnes}>
      ...
      <div className={AddCss.item}>
        <Tabs defaultActiveKey="1"  >
          <TabPane tab="添加视图接口权限" key="1">
            <Form {...layout} name="nest-messages" onFinish={(values: any) => { AddUserStore.addAuthorityViewAction(AddUserStore.ViewAuthorityList[values.gender].view_authority_text, AddUserStore.ViewAuthorityList[values.gender].view_id); formRefFour.resetFields() }} initialValues={{ addAuthorityView: '' }} form={formRefFour}>
      ...
      <div className={AddCss.item}>
        <Tabs defaultActiveKey="1"  >
          <TabPane tab="给身份设置api接口权限" key="1">
            <Form name="nest-messages" {...layout} onFinish={(values: any) => { AddUserStore.setIdentityViewAction(values.user.identity_id, values.user.identity_id.view_authority_id) }} form={formRefFive}>
